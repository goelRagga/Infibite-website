...
export async function generateMetadata(): Promise<Metadata> {
  const { bankDetail } = await getBankCreditCardPage('/axis-bank-cards');
  const page = bankDetail?.page?.seo;
  const slug = page?.slug || 'axis-bank-cards';
  return await generatePageSeo({ page, slug });
}

export default async function BankPage() {
  const { bankDetail, error } = await getBankCreditCardPage('/axis-bank-cards');
